...
namespace ForagePointers
{
    class ModEntry : Mod
    {
        ...
        public override void Entry(IModHelper helper)
        {
            ...
            ScalingFactor = Clamp(Config.ScalingRadius, 0, 50);
        ...
        private void OnHudRendering(object sender, RenderingHudEventArgs e)
        {
            if (Context.IsWorldReady)
            {
                ...
                var viewDist = Math.Pow(MinimumViewDistance + (Game1.player.ForagingLevel / ScaleEveryNLevels * ScalingFactor), 2);
                
                foreach (var v in Game1.currentLocation.objects.Pairs)
                {
                    if ((v.Value.isSpawnedObject || v.Value.ParentSheetIndex == 590) && Utility.isOnScreen(v.Key * 64f + new Vector2(32f, 32f), 64))
                    {
                        ...
                        if (drawArrow)
                        {
                            ...
                            Game1.spriteBatch.Draw(Game1.mouseCursors, trackerRenderPosition, new Rectangle?(srcRect), Color.White, (float)Math.PI, new Vector2(2f, 2f), renderScale, SpriteEffects.None, 1f);
                        }
                    }
                }
            }
